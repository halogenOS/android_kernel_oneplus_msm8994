/*
 * haloplug - Hotplug for best battery-performance ratio
 * Copyright (C) 2012 Alucard_24@XDA
 ...
 *  - Simao Gomes Viana (xdevs23)
 *  - Alucard24@XDA
 *
 * Based on Alucard hotplug
 */
...
static struct workqueue_struct *haloplughp_wq;

static struct delayed_work haloplug_hotplug_work;
...
static void __ref hotplug_work_fn(struct work_struct *work)
{
...
#ifdef CONFIG_haloplug_HOTPLUG_USE_CPU_UTIL
	...
	queue_delayed_work_on(0, haloplughp_wq, &haloplug_hotplug_work,
							  delay);
}

#if defined(CONFIG_POWERSUSPEND) || defined(CONFIG_HAS_EARLYSUSPEND)
#ifdef CONFIG_POWERSUSPEND
static void __ref haloplug_hotplug_suspend(struct power_suspend *handler)
#else
static void __ref haloplug_hotplug_early_suspend(struct early_suspend *handler)
#endif
{
...
#ifdef CONFIG_POWERSUSPEND
static void __ref haloplug_hotplug_resume(struct power_suspend *handler)
#else
static void __ref haloplug_hotplug_late_resume(
...
#ifdef CONFIG_POWERSUSPEND
static struct power_suspend haloplug_hotplug_power_suspend_driver = {
	.suspend = haloplug_hotplug_suspend,
	.resume = haloplug_hotplug_resume,
};
#else
static struct early_suspend haloplug_hotplug_early_suspend_driver = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 10,
	.suspend = haloplug_hotplug_early_suspend,
	.resume = haloplug_hotplug_late_resume,
};
#endif
...
static int hotplug_start(void)
{
	...
	haloplughp_wq = alloc_workqueue("haloplughp_wq", WQ_HIGHPRI, 0);

	if (!haloplughp_wq) {
		printk(KERN_ERR "Failed to create haloplug hotplug workqueue\n");
		return -EFAULT;
	}

	ret = init_rq_avg();
	if (ret) {
		destroy_workqueue(haloplughp_wq);
		return ret;
	}
	...
	init_rq_avg_stats();
	INIT_DELAYED_WORK(&haloplug_hotplug_work, hotplug_work_fn);
	queue_delayed_work_on(0, haloplughp_wq, &haloplug_hotplug_work,
...
#if defined(CONFIG_POWERSUSPEND)
	register_power_suspend(&haloplug_hotplug_power_suspend_driver);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	register_early_suspend(&haloplug_hotplug_early_suspend_driver);
#endif  /* CONFIG_POWERSUSPEND || CONFIG_HAS_EARLYSUSPEND */
...
#if defined(CONFIG_POWERSUSPEND)
	unregister_power_suspend(&haloplug_hotplug_power_suspend_driver);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	unregister_early_suspend(&haloplug_hotplug_early_suspend_driver);
#endif  /* CONFIG_POWERSUSPEND || CONFIG_HAS_EARLYSUSPEND */

	cancel_delayed_work_sync(&haloplug_hotplug_work);
...
static struct attribute *haloplug_hotplug_attributes[] = {
...
static struct attribute_group haloplug_hotplug_attr_group = {
	.attrs = haloplug_hotplug_attributes,
	.name = "haloplug_hotplug",
};

static int __init haloplug_hotplug_init(void)
{
	int ret;
	unsigned int cpu;
...
static void __exit haloplug_hotplug_exit(void)
{
	if (hotplug_tuners_ins.hotplug_enable > 0) {
		hotplug_stop();
	}
...
late_initcall(haloplug_hotplug_init);
